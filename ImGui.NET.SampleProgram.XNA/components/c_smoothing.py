...
class Component():
    ...
    def run(self, image_data, component_values, width, size):
        ...
        is_vertical = component_values[0]
        is_diagonal = component_values[0]
        ...
        for index in range(0, len(image_data)):
            ...
            if (is_vertical):
                ...
                px_r += ( (0 if case_u else image_data[index-1][0]) + (0 if case_d else image_data[index+1][0]) )
                px_g += ( (0 if case_u else image_data[index-1][1]) + (0 if case_d else image_data[index+1][1]) )
                px_b += ( (0 if case_u else image_data[index-1][2]) + (0 if case_d else image_data[index+1][2]) )
